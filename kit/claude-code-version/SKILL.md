...
5. Meal plan: 5 dinners. Locked night stays locked. At least 4 from winners, at most 1 experiment from approved sources only, nothing from the last 2 weeks of history. 2 swaps on the bench. Lunch and breakfast plan per rotation.
6. Output the SUNDAY BRIEF template. Wait for picks.
7. Write back: picks to history, new senders to family.md, new dates to trackers.
...
3. Grocery list: picked meals plus restock items due, split by store in shopping order, warehouse store only on its week.
